...
paths=['index.html','assets/dc019-equipment.glb'];before={p:sha(R/p)for p in paths}
run(['python','build.py'],'build-final.txt');run(['python','tools/export_contact.py'],'export-final.json')
...
run(['node','--test','tests/contact-articulation.test.cjs','tests/palm-surface.test.cjs','tests/contact-transitions.test.cjs'],'contact-final.txt')
...
run(['python','tests/contact_exports.test.py'],'export-test.txt')
...
expected={'handling-browser.json':51,'arsenal-browser.json':78,'library-regression.json':64,'legacy/browser-report.json':52,'arsenal-continuous.json':26};browser=[]
...
clip=json.loads((Q/'clip-report.json').read_text());assert clip['sha256']==H and not clip['errors'] and all(x['gl']==0 for x in clip['states']);assert (Q/'contacto.mp4').is_file()
...
source={p.relative_to(R).as_posix():sha(p)for folder in ['src','tools','tests','docs/v019']for p in sorted((R/folder).rglob('*'))if p.is_file() and '__pycache__' not in p.parts}
source.update({p:sha(R/p)for p in ['README.md','build.py']})
report={'version':'0.19','artifacts':{p:{'bytes':(R/p).stat().st_size,'sha256':sha(R/p)}for p in paths},'logic':{'passed':passes,'failed':0},'browser':{'passed':sum(x['passed']for x in browser),'suites':browser},'exportTests':1,'captures':captures,'clip':{'frames':clip['frames'],'duration':clip['duration'],'sha256':sha(Q/'contacto.mp4')},'sourceHashes':source,'scope':scope,'rebuildIdentical':True,'note':'Browser reports were executed separately. This audit rebuilds, runs Node and export tests, and verifies hashes. Not a GPU hardware or native persistence benchmark.'}
