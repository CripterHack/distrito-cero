...
class GLB:
 def __init__(self,name):
  self.doc={'asset':{'version':'2.0','generator':'Distrito Cero native exporter v0.8','copyright':'Original Distrito Cero geometry'},'scene':0,'scenes':[{'nodes':[0]}],'nodes':[{'name':name,'children':[]}],'meshes':[],'materials':[],'buffers':[{'byteLength':0}],'bufferViews':[],'accessors':[], 'extras':{'provenance':'Native original v0.8 character refinement and existing native coupe recipe. Not a newly downloaded Higgsfield asset. Shader blinks, crowd variants and vehicle interactions are not exported as clips.','units':'metres, +Y up, +Z forward'}};self.binary=bytearray()
...
def hero():
 g=GLB('DC08 native protagonist');bones=D['bones'];ids={b[0]:i+1 for i,b in enumerate(bones)}
 ...
 for part in D['hero']['parts']:
  ...
  rough={30:.65,31:.86,32:.91,33:.84,34:.16,3:.32,4:.82}.get(part['material'],.75)
 ...
 return g.write(ROOT/'assets/dc08-hero-native.glb')

def car():
 g=GLB('DC08 native coupe geometry kit');color=g.material('Petrol enamel',[.055,.20,.24],.26,.58);glass=g.material('Smoked glass',[.028,.069,.095],.10,.28);rubber=g.material('Rubber',[.018,.023,.028],.84);alloy=g.material('Brushed alloy',[.35,.4,.44],.32,.8)
 ...
 return g.write(ROOT/'assets/dc08-coupe-native.glb')
if __name__=='__main__':
 report=[hero(),car()];(ROOT/'assets/presence-exports.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
