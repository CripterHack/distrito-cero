...
const defs=[['Idle',{},4],['Walk',{moveSpeed:3.3},Math.PI*2/(3.3*4.8)],['Run',{moveSpeed:6.2,sprintBlend:1},Math.PI*2/(6.2*2.7)],['Crouch',{crouch:1},3],['Carry',{carry:'crate',reach:.8},3],['HandRelaxed',{grip:0},2],['HandGrip',{grip:1},2]];
...
def hero():
 g=GLB('DC010 articulated constitution');bones=D['bones'];ids={b[0]:i+1 for i,b in enumerate(bones)}
 ...
 for part in D['hero']['parts']:
  ...
  rough={30:.55,31:.86,32:.91,33:.67,34:.16,40:1,41:.19,42:.72,3:.32,4:.82,44:.48}.get(part['material'],.75)
 ...
 g.doc['extras']['limitations']='Native vertex blink, gaze, scalp stipple, crowd morphology, microdetail and neck blend are runtime-only. Material appearance is portable approximation; no motion capture or physical cloth.'
 return g.write(ROOT/'assets/dc010-human-articulated.glb')

if __name__=='__main__':
 report=hero();(ROOT/'assets/body-export.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
