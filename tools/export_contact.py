...
for i,obj in enumerate(objects):
 ...
 for part in obj['parts']:
  ...
  mesh=len(g['meshes']);g['meshes'].append({'name':obj['id']+'_'+part['name'],'primitives':[{'attributes':accessors,'material':material_cache[key],'mode':4}]});g['nodes'][root]['children'].append(len(g['nodes']));g['nodes'].append({'name':obj['id']+'_'+part['name'],'mesh':mesh,'extras':{'role':part.get('role','body')}})
g['buffers']=[{'byteLength':len(buf)}];header=json.dumps(g,separators=(',',':'),ensure_ascii=True).encode();header+=b' '*((-len(header))%4);buf+=b'\x00'*((-len(buf))%4);out=struct.pack('<4sII',b'glTF',2,28+len(header)+len(buf))+struct.pack('<I4s',len(header),b'JSON')+header+struct.pack('<I4s',len(buf),b'BIN\0')+buf;(R/'assets/dc019-equipment.glb').write_bytes(out);print(json.dumps({'bytes':len(out),'equipment':len(objects),'meshes':len(g['meshes']),'triangles':sum(len(p['data'])//24 for o in objects for p in o['parts'])}))
