#!/usr/bin/env python3
"""v0.9: licensed MakeHuman HM08 anatomy, baked skin maps and original garments.
Build inputs and CC0 provenance are in assets/anatomy-source. No network at build
time or runtime. Reuses the established 17-bone skeleton and gameplay poses.
"""
import base64, json, struct, hashlib, lzma
...
def weights(cat,p):
 ...
 if cat=='jacket':
  ...
  t=float(np.clip((abs(x)-.187)/.085,0,1));t=t*t*(3-2*t)
  out={}
...
def union_garment(name,lofts,material,step=.016):
 ...
 field=gaussian_filter(field,.68)
...
TORSO=[(0,.96,0,.175,.112),(0,1,0,.186,.12),(0,1.07,0,.178,.112),(0,1.16,-.003,.193,.121),(0,1.28,-.009,.210,.115),(0,1.39,-.008,.241,.108),(0,1.43,-.008,.241,.102),(0,1.455,-.008,.216,.089),(0,1.48,-.008,.132,.071),(0,1.496,-.008,.063,.056)]
SLEEVE=[(1.468,.213,.019,.027),(1.454,.224,.043,.060),(1.45,.233,.065,.072),(1.40,.254,.070,.078),(1.31,.269,.063,.073),(1.22,.28,.067,.075),(1.12,.29,.063,.070),(1.065,.294,.065,.072),(.97,.30,.058,.064),(.87,.305,.050,.055),(.835,.305,.046,.051)]
...
LEG=[(.96,.09,0,.098,.109),(.86,.112,0,.105,.116),(.76,.116,-.003,.087,.099),(.63,.116,-.004,.077,.087),(.49,.115,.005,.068,.076),(.44,.115,.003,.072,.081),(.36,.115,-.008,.069,.081),(.23,.115,-.004,.057,.067),(.10,.115,0,.048,.057),(.075,.115,0,.049,.056)]
...
for side in [-1,1]:
 ...
 for y in np.linspace(.98,1.478,26):
  z=float(np.interp(y,[q[1]for q in TORSO],[q[2]+q[4]for q in TORSO]))-.001
  rings.append((side*.005,y,z,.0020,.0012))
 loft('Zip tape'+str(side),rings,'pants','jacket',8,1)
for i in range(40):
 y=.985+i*.0123;z=float(np.interp(y,[q[1] for q in TORSO],[q[2]+q[4] for q in TORSO]))+.0006
 ellipsoid('zip'+str(i),((-1 if i%2 else 1)*.0016,y,z),(.0017,.0014,.001),'metal','jacket',8,4)
for side in [-1,1]:
 k='L'if side<0 else'R'
 loft('Rib cuff '+k,[(side*.305,.832,0,.047,.052),(side*.305,.855,0,.048,.053),(side*.304,.875,0,.049,.054)],'pants','jacket',24,2)
 # Pocket piping sunk into the cloth on a slightly slanted line.
 for j in range(12):
  x=side*(.071+j*.0063);y=1.11-j*.0024;z=.111*np.sqrt(max(.05,1-(x/.183)**2))+.002
...
meta={'version':1,'stride':24,'positionScale':10000,'bones':[n for n,_,_ in BONES],'parts':parts,'provenance':'MakeHuman HM08 CC0 anatomical head with adult target, Mindfront Aksel CC0 skin maps. Original native garments, hands, eyes and groom. Authoring tools only, no external runtime libraries.'}
...
report={'triangles':sum(x['vertices']//3 for x in parts),'parts':[{k:v for k,v in p.items() if k!='data'} for p in parts],'components':METRICS,'historical_recipe_project':'bea56607-3eea-4526-abc9-b546a4717f10','integration':'v0.9: actual CC0 HM08 head subset integrated in native rig. Skin bake prepared through Higgsfield sandbox, not an AI scan.'}
