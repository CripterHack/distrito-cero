...
for(const n of ['core','character-fit','character-motion','skin-rig','hero-asset','hair-geometry','visual-geometry'])vm.runInThisContext(fs.readFileSync('src/'+n+'.js','utf8'));
const drop=DC.CharacterFit.drop(0);
function decode(part){const b=Buffer.from(part.data,'base64'),out=new Float32Array(part.vertices*16);for(let i=0;i<part.vertices;i++){const p=i*24,o=i*16;for(let j=0;j<3;j++){out[o+j]=b.readInt16LE(p+j*2)/1e4;out[o+3+j]=b.readInt16LE(p+6+j*2)/32767;}for(let j=0;j<2;j++)out[o+6+j]=b.readUInt16LE(p+12+j*2)/65535;for(let j=0;j<4;j++){out[o+8+j]=b[p+16+j];out[o+12+j]=b[p+20+j]/255;}}return out;}
function encode(name,material,color,rows){const b=Buffer.alloc(rows.length/16*24);for(let o=0;o<rows.length;o+=16){const k=o/16*24,hair=material===46||material===47;const pos=hair?[rows[o],rows[o+1]-drop,rows[o+2]]:DC.CharacterFit.point(Array.from(rows.subarray(o,o+3)),drop);let n=hair?Array.from(rows.subarray(o+3,o+6)):DC.CharacterFit.normal(Array.from(rows.subarray(o+3,o+6)),rows[o+1],drop);const len=Math.hypot(...n);n=n.map(v=>v/len);for(let j=0;j<3;j++){b.writeInt16LE(Math.round(pos[j]*1e4),k+j*2);b.writeInt16LE(Math.round(n[j]*32767),k+6+j*2);}for(let j=0;j<2;j++)b.writeUInt16LE(Math.round(Math.max(0,Math.min(1,rows[o+6+j]))*65535),k+12+j*2);for(let j=0;j<4;j++){b[k+16+j]=rows[o+8+j];b[k+20+j]=Math.round(rows[o+12+j]*255);}}
return{name,material,color,vertices:rows.length/16,data:b.toString('base64')};}
const fitted=[];for(const part of DC.HeroAsset.parts){if(part.name==='scalp')continue;let rows=decode(part);if(part.name==='hair')rows=DC.HairGeometry.brows(rows);fitted.push(encode(part.name,part.material,part.color,rows));}
for(const p of DC.HairGeometry.build(0,0))fitted.push(encode('groom_'+p.name,p.material,[.063,.033,.017],p.data));
const hero={...DC.HeroAsset,parts:fitted};
...
const clips=defs.map(([name,state,duration])=>{const frames=[];const count=Math.max(18,Math.ceil(duration*30));for(let i=0;i<=count;i++){
 const t=i/count,pose=DC.SkinRig.pose({...state,neckDrop:drop,...(name==='NeckTurn'?{lookYaw:Math.sin(t*Math.PI*2)*.72}:name==='NeckNod'?{lookPitch:Math.sin(t*Math.PI*2)*.22}:{}),walk:name==='Walk'||name==='Run'?t*Math.PI*2:0},t*duration);
...
def hero():
 g=GLB('DC016 Rasgos neutral fitted neck and classic groom');bones=D['bones'];ids={b[0]:i+1 for i,b in enumerate(bones)}
 ...
 for part in D['hero']['parts']:
  ...
  rough={30:.66,31:.83,32:.90,33:.67,34:.12,40:1,41:.19,42:.72,3:.32,4:.82,44:.56,46:.67,47:.84}.get(part['material'],.75)
  ...
  if part['material'] in (33,42,46,47):g.doc['materials'][mat]['doubleSided']=True
 ...
 return g.write(ROOT/'assets/dc016-human-traits.glb')

if __name__=='__main__':
 report=hero();(ROOT/'assets/traits-export.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
