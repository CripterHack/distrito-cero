...
out=Path(sys.argv[1]) if len(sys.argv)>1 else R.parent/'distrito-cero-v0.18-codigo.zip'
out=out.resolve()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
report=json.loads((R/'qa/v018/release-report.json').read_text())
assert sha(R/'index.html')==report['artifacts']['index.html']['sha256']
assert sha(R/'assets/dc018-equipment.glb')==report['artifacts']['assets/dc018-equipment.glb']['sha256']
...
for p in sorted(R.rglob('*')):
 ...
 if any(x in {'.git','node_modules','__pycache__','handling-frames'} for x in p.relative_to(R).parts):continue
 if p.name in {'SOURCE-MANIFEST.json','.DS_Store'} or p.suffix.lower() in {'.pyc','.pid','.ttf','.otf','.woff','.woff2'}:continue
 files.append(p)
manifest={'version':'0.18','htmlSha256':sha(R/'index.html'),'files':{p.relative_to(R).as_posix():sha(p) for p in files}}
