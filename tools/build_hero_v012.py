#!/usr/bin/env python3
"""v0.12: licensed MakeHuman HM08 head, cervical surface and original garments.
Build inputs and CC0 provenance are in assets/anatomy-source. No network at build
time or runtime. Keeps the 49-bone contract and refines shoulders, neck, wrist and hand surfaces.
"""
import base64, json, struct, hashlib, lzma, subprocess
import body_authoring_v012 as BODY
...
def weights(cat,p):
 ...
 if cat=='anatomical':
  if y<1.526:return blend('chest','neck',(y-1.453)/.073)
  return blend('neck','head',(y-1.526)/.072) if y<1.598 else {'head':1}
 if cat=='head':return {'head':1}
 if cat=='neck':return {'chest':1}
...
def union_garment(name,lofts,material,step=.0125):
 from scipy.interpolate import PchipInterpolator
 ...
 for rings in lofts:
  ...
  f=np.maximum(f,np.maximum(r[0,1]-y,y-r[-1,1]))
  if material=='jacket':
   # Polynomial smooth union creates a broad cloth bridge through the axilla/deltoid.
   k=.026;h=np.maximum(k-np.abs(field-f),0)/k;field=np.minimum(field,f)-h*h*k*.25
  else:field=np.minimum(field,f)
 if material=='jacket':
  # Real neck opening: remove central top rather than placing a solid cap behind the neck.
  hole=(np.sqrt((x/.067)**2+((z+.010)/.059)**2)-1)*.059
  field=np.maximum(field,-np.maximum(hole,1.445-y))
 ...
 for i,point in enumerate(v):
  ...
  point[2]+=.0015*fold*(np.sin(yy*78+xx*26)+.45*np.sin(yy*131-xx*41))*np.sin(angle)
...
# The licensed head subset includes the start of the shoulder flare. It is not
# cervical anatomy: narrow that inherited lateral spike before bridging the neck.
# Preserve jaw/chin above the transition and keep topology/UV correspondence.
for i,point in enumerate(v):
 x,y,z=point
 if y>=1.556:continue
 t=float(np.clip((y-1.528)/.028,0,1));amount=1-t*t*(3-2*t)
 phi=np.arctan2(x,z+.010)
 rx=float(np.interp(y,[1.51,1.531,1.545,1.556],[.046,.045,.050,.056]))
 rz=float(np.interp(y,[1.51,1.531,1.545,1.556],[.047,.046,.052,.063]))
 # Do not flatten the chin which begins in the front of this height range.
 chin=float(np.clip((z-.045)/.036,0,1))*float(np.clip((y-1.533)/.016,0,1))
 amount*=1-chin
 v[i]=[x*(1-amount)+np.sin(phi)*rx*amount,y,z*(1-amount)+(-.010+np.cos(phi)*rz)*amount]
...
for j in range(1,13):
 ...
 for i,origin in initial.items():
  t=j/12;phi=np.arctan2(origin[0],origin[2]+.010);y=origin[1]*(1-t)+1.447*t
  # Non-cylindrical neck with paired sternomastoid ridges, wider cervical base,
  # restrained laryngeal prominence. Continuous with the actual HM08 boundary.
  yy=np.array([1.447,1.478,1.513,1.545,1.572])
  rx=np.interp(y,yy,[.073,.058,.046,.043,.049]);rz=np.interp(y,yy,[.066,.056,.046,.047,.053])
  scm=.0023*np.exp(-((abs(phi)-(.28+.93*np.clip((y-1.466)/.10,0,1)))/.19)**2)*np.sin(np.pi*t)**2
  larynx=.0017*np.exp(-(phi/.26)**2-((y-1.544)/.014)**2)
  desired=np.array([np.sin(phi)*(rx+scm),y,-.010+np.cos(phi)*(rz+scm+larynx)])
  blend_t=1-(1-t)**3;linear=origin.copy();linear[1]=y
  pt=linear*(1-blend_t)+desired*blend_t
...
# Open rib-knit neckline, lower in front, not a capped cylinder.
p=[];f=[];N=64
for j in range(4):
 for i in range(N):
  a=2*PI*i/N;front=max(0,np.sin(a));y=1.475+.010*(1-front)-(.012 if j<2 else 0)
  rx=.073-(.005 if j%2 else 0);rz=.066-(.005 if j%2 else 0)
  p.append([np.cos(a)*rx,y,-.010+np.sin(a)*rz])
for j in range(3):
 for i in range(N):
  a=j*N+i;b=j*N+(i+1)%N;f.extend([[a,b,b+N],[a,b+N,a+N]])
add_mesh('Open anatomical neckline',p,f,'jacket','neck')
...
meta={'version':2,'stride':24,'positionScale':10000,'bones':[n for n,_,_ in BONES],'parts':parts,'provenance':'MakeHuman HM08 CC0 anatomical head with adult target, Mindfront Aksel CC0 skin maps. Original v0.12 connected-shoulder and cervical refinement with continuous articulated hands, tailored garments, shoe lasts, eyes and groom. Authoring tools only, no external runtime libraries.'}
...
report={'triangles':sum(x['vertices']//3 for x in parts),'parts':[{k:v for k,v in p.items() if k!='data'} for p in parts],'components':METRICS,'historical_recipe_project':'bea56607-3eea-4526-abc9-b546a4717f10','integration':'v0.12 (head preserved from v0.9): actual CC0 HM08 head subset integrated in native rig. Skin bake prepared through Higgsfield sandbox, not an AI scan.'}
