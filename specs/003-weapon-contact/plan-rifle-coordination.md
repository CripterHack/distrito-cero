...
**Decisión:** conservar el requisito fallido y no integrar los prototipos. Coste: el rifle sigue desalineado, pero master no recibe una postura que sólo pasa números. No crear otra matriz QA equivalente. Retomar desde esta regresión, revisar la superficie del hombro y resolver conjuntamente ambos hombros, cabeza y montaje antes de extender familias.
...
- [ ] Fijar límites de articulaciones/superficies y referencias permitidas. Incorporar un control que rechace las variantes anteriores antes de seleccionar otra pose.
- [ ] Implementar una coordinación acotada dentro de `src/weapon-handling.js` y `src/skin-rig.js`, no otro montaje. Las fuentes humanas y las longitudes se conservan. Cualquier necesidad de cambiar geometría/referencias requiere explicitar un contrato diferente antes de hacerlo.
...
- [ ] Build/export, Node completo, Python vigente, suites gráficas y HTTP aplicables. Comparación del mismo renderer sin sustituir sus módulos, más superficie de la prenda y continuidad.
- [ ] Actualizar STATE/HANDOFF/QA con una corrección real, revisar diff y CI del HEAD exacto antes de solicitar merge. Después verificar master y Pages por separado.
...
Los dos primeros comandos deben fallar por el requisito ocular mientras no exista corrección. El tercero debe aprobar sin mutar archivos. El PR permanece en borrador. No usar `skip`, bajar el criterio, retirar el test de CI ni integrar sólo porque las suites antiguas sigan pasando.

La copia local procede del artefacto Pages `10649968865`, ZIP SHA-256 `d14dbc643645e4121f757a6a958fd432510de98885efa3821deb41e22c0394c7`. Git remoto no resolvió DNS. Se publican commits por el conector sobre padres remotos reales, nunca historia sintética del laboratorio. Los prototipos se ejecutaron con Chromium del sistema, Xvfb y GPU software, sin benchmark físico ni prueba nueva de navegación HTTP pública.
