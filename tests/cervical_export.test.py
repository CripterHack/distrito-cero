"""Structural checks on the actual v0.13 textured authoring export, not historical GLBs."""
...
class HumanExport(unittest.TestCase):
 def test_textured_rigged_export(self):
  f=R/'assets/dc014-human-cervical.glb'
  self.assertTrue(f.is_file(),'v0.13 textured export is missing')
  ...
  self.assertEqual(len(d['meshes']),13);self.assertEqual(len(d['skins'][0]['joints']),49);self.assertEqual(len(d['animations']),11)
  ...
  for mesh in d['meshes']:
   ...
   self.assertLess(int(read(p['indices']).max()),len(pos));self.assertTrue((read(a['WEIGHTS_0']).sum(1)==255).all());self.assertLess(int(read(a['JOINTS_0']).max()),411)
