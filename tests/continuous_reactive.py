"""Live v0.4 renderer + keyboard + simulation. No renderer/storage/gameplay substitutes.
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'qa/v04';errors=[];requests=[];checks=[]
def check(name,v):
...
try:
 ...
 report={'checks':checks,'passed':sum(x['passed'] for x in checks),'failed':sum(not x['passed'] for x in checks),'before_police':locals().get('before'),'after_police':locals().get('after'),'lamp_hit':locals().get('hit'),'cleanup':'Owned test browser terminated after all assertions to avoid the lab SwiftShader shutdown hang','renderer_replaced':False,'storage_replaced':False,'simulation_replaced':False,'errors':errors,'requests':requests,'environment':'Chromium, Xvfb, SwiftShader software GPU. Live loop at 640x400 eco, review screenshot 1280x800 high. set_content. Not a hardware benchmark.'}
 (OUT/'continuous-reactive-report.json').write_text(json.dumps(report,indent=2,ensure_ascii=False));print(json.dumps(report,indent=2,ensure_ascii=False),flush=True)
