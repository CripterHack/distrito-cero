...
class HumanExport(unittest.TestCase):
 def test_textured_rigged_export(self):
  f=R/'assets/dc013-human-motion.glb'
  ...
  self.assertEqual(len(d['meshes']),13);self.assertEqual(len(d['skins'][0]['joints']),49);self.assertEqual(len(d['animations']),9)
  ...
  for anim in d['animations']:
   ...
   self.assertIn(anim['name'].split()[0], ['Idle','Walk','Run','Crouch','Carry','HandRelaxed','HandGrip','Seated','Reach'])
