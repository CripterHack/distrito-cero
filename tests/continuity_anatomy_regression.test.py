...
class Anatomy(unittest.TestCase):
 ...
 def test_anatomical_dimensions_and_neck_connection(self):
  a=part('face');p=a['pos']/10000.;self.assertAlmostEqual(p[:,1].min(),1.447,places=3);self.assertTrue(1.77<p[:,1].max()<1.80);self.assertTrue(.17<np.ptp(p[:,0])<.21)
  for low in [1.455,1.49,1.51,1.535]:self.assertGreater(np.sum((p[:,1]>=low-.006)&(p[:,1]<low+.021)),15)
