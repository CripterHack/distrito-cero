...
R=Path(__file__).resolve().parents[1];Q=R/'qa/v018';O=Q/'handling-frames';O.mkdir(exist_ok=True)
...
subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-framerate','20','-i',str(O/'%04d.png'),'-c:v','libx264','-pix_fmt','yuv420p','-crf','20','-movflags','+faststart',str(Q/'manejo.mp4')],check=True)
