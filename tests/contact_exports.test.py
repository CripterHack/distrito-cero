...
class ContactExport(unittest.TestCase):
 def test_original_kit_matches_exported_node_catalogue(self):
  ...
  self.assertEqual(len(g['meshes']),78);self.assertEqual(sum(n.get('extras',{}).get('role')=='grip' for n in g['nodes']),9);self.assertEqual(sum(n.get('extras',{}).get('role')=='magazine' for n in g['nodes']),6);self.assertEqual(g['asset']['version'],'2.0');self.assertFalse(g.get('extensionsRequired'))
