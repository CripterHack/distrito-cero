"""v0.8 DOM/paired-animation/streaming regression. Actual WebGL keyframes.
...
R=Path(__file__).resolve().parents[1];O=R/'qa/v08';O.mkdir(exist_ok=True);os.environ.setdefault('DISPLAY',':99')
...
def load(b,w,h,touch=False):
 c=b.new_context(viewport={'width':w,'height':h},has_touch=touch,is_mobile=touch,device_scale_factor=1);p=c.new_page();p.set_default_timeout(30000);p.on('pageerror',lambda e:errors.append(str(e)));p.on('console',lambda m:errors.append(m.text)if m.type=='error'else None);p.on('request',lambda r:requests.append(r.url));p.set_content((R/'index.html').read_text().replace('<script>','<script>'+FIXTURE,1),timeout=60000);p.wait_for_function('!!window.DC_APP',timeout=60000)
...
try:
 with sync_playwright() as pw:
  ...
  ck('GPU material groups are shared, not one draw per NPC',p.evaluate('DC_APP.renderer.castStats.drawBatches===11&&DC_APP.renderer.castStats.actors===2'))
  ...
  ck('Draw batches stay bounded by material and LOD',p.evaluate('DC_APP.renderer.castStats.drawBatches<=33'))
  ...
  ck('Palettes differ between people',p.evaluate('const a=DC_APP.renderer.posePixels;a.slice(272,544).some((v,i)=>Math.abs(v-a[544+i])>.001)'))
  ck('Bone texture cannot grow with explored world',p.evaluate('DC_APP.renderer.visualStats.crowdPaletteBytes===139264&&DC_APP.renderer.castStats.paletteRows<=128'))
