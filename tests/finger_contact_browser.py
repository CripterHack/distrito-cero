"""Finger contact surfaces in the production renderer. Prepared scenes and clock.
No hardware FPS claim. Uses an explicit storage fixture, not native persistence.
A baseline HTML can be captured separately via DC_FINGER_HTML for visual review.
"""
...
O=Path(os.environ.get('DC_FINGER_OUTPUT', str(R/('qa/v019/finger-contact' if os.environ.get('DC_QA_RUN_ID') else 'artifacts/fingers-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')))))
O.mkdir(parents=True,exist_ok=False)
html=Path(os.environ.get('DC_FINGER_HTML',str(R/'index.html'))).read_text()
sha=hashlib.sha256(html.encode()).hexdigest();checks=[];errors=[];requests=[];samples=[]
comparison=bool(os.environ.get('DC_FINGER_HTML'))
 ...
 with sync_playwright() as pw:
  ...
  for item,side,volume,progress in cases:
   value=p.evaluate('''c=>{const a=DC_APP,s=a.sim,r=a.renderer;s.equipWeapon(c.item);s.equipment.handling.ready=1;s.time=1.25;s.player.crouch=0;s.equipment.aimWeight=1;s.equipment.pitch=0;s.equipment.reloading=c.progress?(1-c.progress)*DC.Equipment.get(c.item).reload:0;r.motionTracker.clear();r.motionScene=s;
    r.render(s);let m=qaDrawnMount,palm=m.palmContacts[c.side],offset=m.direction([c.side==='R'?.23:-.23,-.16,.25]);r.camera.target=[palm.x,palm.y-.015,palm.z+.01];r.camera.eye=r.camera.target.map((v,i)=>v+offset[i]);r.render(s);r.gl.finish();
    const q={matrices:r.heroPalette,rootY:r.motionDebug.rootY,scale:1},names=c.side==='R'?['middle','ring','little']:['index','middle','ring','little'];return{minimums:Object.fromEntries(names.map(n=>[n,DC_FINGER_QA.stats(s,c.side,n,c.volume,{mount:qaDrawnMount,pose:q})])),gl:r.gl.getError(),contacts:r.equipmentStats.contacts,frame:r.frame};}''',{'item':item,'side':side,'volume':volume,'progress':progress})
   name=f'{item}-{side}-{volume}';samples.append({'name':name,**value})
   ck(name+' rendered skin stays outside the contact volume',all(v['min']>-.003 and v['min']<.007 for v in value['minimums'].values()))
  ...
  p.evaluate('''()=>{const a=DC_APP,s=a.sim;s.equipWeapon('rifle');s.equipment.handling.ready=1;s.equipment.ammo.rifle.loaded=4;window.ammoBefore=s.equipment.ammo.rifle.reserve;s.equipment.reloading=0;s.reloadWeapon();window.lastFingers=null;window.maxFingerStep=0;}''')
  for block in range(12):
   v=p.evaluate('''()=>{const a=DC_APP,s=a.sim;for(let i=0;i<15;i++){s.time+=1/60;s.equipmentStep(1/60,{aim:true});const m=DC.Equipment.mount(s),f=['index','middle','ring','little'].flatMap(n=>m.grips.L.fingers[n]);if(lastFingers)maxFingerStep=Math.max(maxFingerStep,...f.map((v,j)=>Math.abs(v-lastFingers[j])));lastFingers=f;}a.renderer.render(s);a.renderer.gl.finish();return a.renderer.gl.getError();}''')
   assert v==0
   if block in [0,2,4,6,8,11]:p.screenshot(path=str(O/f'reload-{block:02}.png'))
  ck('Reload interpolates finger flexion instead of snapping between grasp profiles',p.evaluate('maxFingerStep<.25'))
