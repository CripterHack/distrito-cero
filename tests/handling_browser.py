"""v0.18 acceptance: real WebGL keyframes, isolated localStorage, explicit simulation ticks.
The continuous suite separately retains RAF and unmodified renderer/camera/simulation.
"""
from pathlib import Path
import os, json, hashlib
from playwright.sync_api import sync_playwright
R=Path(__file__).resolve().parents[1]; O=R/'qa/v018'; os.environ.setdefault('DISPLAY',':99')
...
def tick(p,n=1,draw=False):
 p.evaluate('''({n,draw})=>{const a=DC_APP,s=a.sim;for(let i=0;i<n;i++)if(a.mode==='play'){s.step(1/60,a.input());a.renderer.updateCamera(s,1/60);}a.updateUI(performance.now());if(draw)a.renderer.render(s);}''',{'n':n,'draw':draw})
...
try:
 with sync_playwright() as pw:
  b=pw.chromium.launch(executable_path='/usr/bin/chromium',headless=False,args=['--no-sandbox','--use-angle=swiftshader','--enable-unsafe-swiftshader','--disable-dev-shm-usage']);p=b.new_page(viewport={'width':1120,'height':740});p.set_default_timeout(30000)
  ...
  p.set_content(HTML.replace('<script>','<script>'+FIX,1),timeout=90000);p.wait_for_function('!!window.DC_APP',timeout=90000);p.evaluate('DC_APP.renderer.humanReady');p.evaluate('window.requestAnimationFrame=()=>0');p.wait_for_timeout(100)
