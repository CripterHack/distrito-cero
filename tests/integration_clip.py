...
R=Path(__file__).resolve().parents[1];O=R/'qa/v015';F=O/'clip-frames';F.mkdir(exist_ok=True)
os.environ.setdefault('DISPLAY',':99');fps=20;per=80;errors=[];requests=[]
...
with sync_playwright() as pw:
 ...
 labels=['GIRO · cabeza y cuello coordinados','FLEXIÓN Y EXTENSIÓN · mandíbula estable']
 for phase in range(2):
  for i in range(per):
   t=i/fps
   out=p.evaluate('''({phase,t})=>{const a=DC_APP,c=a.creator,s=c.preview,r=a.renderer,n=s.player;s.time=t+1;
    ...
    r.camera.eye=phase===0?[.18,1.645,.74]:[.66,1.645,.40];r.camera.target=[0,1.535,0];
    const pose=DC.SkinRig.pose(n,s.time);r.frame=0;drawActual(s);
   ...
   d=ImageDraw.Draw(frame);d.rectangle((0,0,frame.width,66),fill=(12,19,24));d.text((20,12),'DISTRITO CERO  v0.15',fill=(220,233,220),font=font);d.text((20,39),labels[phase],fill=(228,233,237),font=small)
   ...
   if i in [0,20,40,60,79]:samples.append({'phase':phase,'sample':i,**out})
