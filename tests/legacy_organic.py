...
OUT=ROOT/'qa/v013/legacy';OUT.mkdir(parents=True,exist_ok=True)
...
with sync_playwright() as p:
    ...
    check('Save writes a validated snapshot',page.evaluate("DC_APP.getStore().active().data.version===1"))
...
report={'sha256':hashlib.sha256(HTML.encode()).hexdigest(),'environment':'Chromium / software WebGL2 / Xvfb / injected HTML / isolated test-storage fixture','native_safari_tested':False,'native_persistence_tested':False,'errors':errors,'requests':requests,'checks':results}
