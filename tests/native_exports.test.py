...
class Exports(unittest.TestCase):
 def read_asset(self,name):
  ...
  return d,raw
 def test_hero(self):
  d,raw=self.read_asset('dc05-hero-native.glb');self.assertEqual(len(d['skins'][0]['joints']),17);self.assertEqual(len(d['meshes']),11)
 ...
 def test_car(self):
  d,raw=self.read_asset('dc05-coupe-native.glb');self.assertGreaterEqual(len(d['meshes']),5);self.assertGreaterEqual(len(d['nodes']),13)
