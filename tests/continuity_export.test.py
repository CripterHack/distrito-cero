"""Structural checks on the actual v0.12 textured authoring export, not historical GLBs."""
...
class HumanExport(unittest.TestCase):
 def test_textured_rigged_export(self):
  f=R/'assets/dc012-human-continuity.glb'
  self.assertTrue(f.is_file(),'v0.12 textured export is missing')
  ...
  self.assertEqual(len(d['meshes']),13);self.assertEqual(len(d['skins'][0]['joints']),49);self.assertEqual(len(d['animations']),7)
  ...
  for anim in d['animations']:
   self.assertEqual(len(anim['channels']),len(d['skins'][0]['joints'])+1)
