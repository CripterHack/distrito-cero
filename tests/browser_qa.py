...
Continuous software-WebGL was checked separately; this suite freezes ONLY the
...
OUT=ROOT/'qa/v04/legacy';OUT.mkdir(exist_ok=True)
...
def reset_player(page,x,z):
    page.evaluate('''({x,z})=>{const a=DC_APP;if(a.sim.player.car!==null){a.sim.actor().speed=0;a.sim.actor().parked=true;}Object.assign(a.sim.player,{x,z,car:null,y:0,vy:0,vx:0,vz:0,transition:null,dodge:0,stagger:0});a.renderer.camera.initialized=false;a.renderer.camera.yaw=0;a.sim.pin=null;a.sim.events=[];}''',{'x':x,'z':z})
with sync_playwright() as p:
    ...
    page.keyboard.press('f');check('F enters the nearby vehicle',page.evaluate('DC_APP.sim.player.car===0'))
    page.keyboard.down('w');page.wait_for_timeout(1400);page.keyboard.up('w')
    ...
    page.keyboard.down('Space');page.wait_for_timeout(1300);page.keyboard.up('Space');page.keyboard.press('f')
    ...
    reset_player(page,5,8);page.evaluate('Object.assign(DC_APP.sim.cars[0],{x:5,z:11,yaw:0,speed:0,parked:true})');page.keyboard.press('f')
    ...
    reset_player(page,252,-72);page.keyboard.press('e');page.wait_for_timeout(1200)
    ...
    cdp=context.new_cdp_session(page);cdp.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':x,'y':y-25,'id':1}]});page.wait_for_timeout(700);cdp.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]})
...
report={'environment':'Chromium 144 / software WebGL2 / Xvfb / injected HTML / isolated test-storage fixture','native_safari_tested':False,'native_persistence_tested':False,'errors':errors,'requests':requests,'checks':results}
