...
R=Path(__file__).resolve().parents[1];O=R/'qa/v016';O.mkdir(parents=True,exist_ok=True);name=sys.argv[1] if len(sys.argv)>1 else 'full';os.environ.setdefault('DISPLAY',':99')
...
with sync_playwright() as pw:
 ...
 if name.startswith('hair'):
  p.evaluate('i=>{DC_APP.creator.preview.appearance.hairStyle=i;DC_APP.renderer.camera.eye=[.23,1.69,.65];DC_APP.renderer.camera.target=[0,1.62,-.005];}',int(name[4:]))
