...
class Anatomy(unittest.TestCase):
 ...
 def test_uv_seam_and_normal_integrity(self):
  ...
  for d in D['parts']:
   a=part(d['name']);self.assertTrue(np.isfinite(a['pos']).all());self.assertTrue(np.allclose(np.linalg.norm(a['normal']/32767.,axis=1),1,atol=.001));self.assertLess(a['joints'].max(),17);self.assertTrue(np.all(a['weights'].sum(1)==255))
 ...
 def test_authoring_export_uses_expected_family(self):
  self.assertEqual(D['bones'][4],'head');self.assertIn('MakeHuman',D['provenance']);self.assertEqual(len(D['parts']),12);self.assertTrue(50000<sum(len(part(p['name']))//3 for p in D['parts'])<60000)
