...
R=Path(__file__).resolve().parents[1];O=R/'qa/v016';name=sys.argv[1] if len(sys.argv)>1 else 'driver';tag=os.environ.get('DC_REVIEW_TAG','');os.environ.setdefault('DISPLAY',':99')
...
with sync_playwright() as p:
 ...
 if name in ['open','pull','release','enter','exit']:
  ...
  page.evaluate("""()=>{const s=DC_APP.sim;for(let i=0;i<4;i++){const n=s.peds[i];Object.assign(n,{hidden:false,variant:[6,7,9,10][i],x:i*1.05+2.2,z:40.6,skin:DC.driverProfile(s.cars[i+1]).skin,coat:DC.driverProfile(s.cars[i+1]).coat,police:i===3,y:0,yaw:Math.PI*.06,walk:i*1.7,moveSpeed:i%2?1.3:0,speed:0});}s.cars[1].x=10;s.player.x=1;DC_APP.renderer.camera.eye=[3,2.45,47];DC_APP.renderer.camera.target=[3.4,1.15,40];}""")
