"""v0.15 acceptance, genuine DOM + WebGL frames. Memory fixture, not native persistence."""
from pathlib import Path
import os,json,hashlib
from playwright.sync_api import sync_playwright
R=Path(__file__).resolve().parents[1];O=R/'qa/v015';os.environ.setdefault('DISPLAY',':99');checks=[];errors=[];requests=[]
...
try:
 with sync_playwright() as pw:
  ...
  p.evaluate('drawActual(DC_APP.creator.preview)');gpu=p.evaluate((R/'tests/integration_gpu.js').read_text());(O/'gpu-transform-feedback.json').write_text(json.dumps(gpu,indent=2))
  ck('Actual vertex shader agrees with CPU for 72 neck/collar/constitution cases',gpu['maxPositionError']<.00001 and gpu['gl']==0)
 ...
 (O/'integration-browser.json').write_text(json.dumps(report,indent=2,ensure_ascii=False));print('RESULT',report['passed'],report['failed'],flush=True)
