...
R=Path(__file__).resolve().parents[1];O=R/'qa/v08';name=sys.argv[1] if len(sys.argv)>1 else 'driver';os.environ.setdefault('DISPLAY',':99')
...
with sync_playwright() as p:
 b=p.chromium.launch(executable_path='/usr/bin/chromium',headless=False,args=['--no-sandbox','--use-angle=swiftshader','--enable-unsafe-swiftshader','--disable-dev-shm-usage']);page=b.new_page(viewport={'width':1120,'height':740});errors=[];page.on('pageerror',lambda e:errors.append(str(e)));page.set_content((R/'index.html').read_text().replace('<script>','<script>'+FIXTURE,1),timeout=90000);page.wait_for_function('!!window.DC_APP',timeout=90000)
 page.evaluate('window.drawActual=DC_APP.renderer.render.bind(DC_APP.renderer);DC_APP.renderer.render=()=>{};DC_APP.renderer.updateCamera=()=>{};')
 ...
 page.evaluate('DC_APP.renderer.syncSectors(25);DC_APP.renderer.frame=0;drawActual(DC_APP.sim)');report=page.evaluate('({stats:DC_APP.renderer.castStats,visual:DC_APP.renderer.visualStats,access:DC_APP.sim.access?.kind,phase:DC_APP.sim.accessPlayerPose()?.accessPhase,gl:DC_APP.renderer.gl.getError()})');report['errors']=errors;report['source_sha256']=hashlib.sha256((R/'index.html').read_bytes()).hexdigest();print(json.dumps(report),flush=True);page.screenshot(path=str(O/('review-'+name+'.png')),timeout=90000);(O/('review-'+name+'.json')).write_text(json.dumps(report,indent=2));b.close()
