...
try:
 ...
 report={'passed':sum(c['pass']for c in checks),'failed':sum(not c['pass']for c in checks),'checks':checks,'errors':errors,'requests':requests,'observed':locals().get('info'),'sha256':hashlib.sha256((R/'index.html').read_bytes()).hexdigest(),'environment':'Chromium Xvfb SwiftShader 680x440. Unmodified RAF/camera/render/simulation with prepared initial poses and native keys. Isolated test storage. Not a hardware FPS benchmark.'};(O/'organic-continuous.json').write_text(json.dumps(report,indent=2));print('RESULT',report['passed'],report['failed'],flush=True)
