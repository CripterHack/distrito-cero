...
(function(D){
 ...
 class EquipmentApp extends D.App{
  ...
  selectEquipment(id,close=false){if(!E.get(id))return;const changed=id!==this.sim.equipment.selected,actor=changed?this.renderer.handlingActor?.(this.sim):null;
   // Capture the view before input cancellation resets the short-arm aim blend.
   // A paused selector already owns its visible mount; do not recompute it from
   // the cancelled simulation. Only the pose snapshot reaches equipWeapon.
   const frozen=this.renderer.frozenHandling,shown=changed&&this.sim.equipmentAvailable()?(frozen?.equipment.selected===this.sim.equipment.selected?frozen.mount:D.WeaponHandling.present(this.sim,actor)):null;
   this.clearWeaponInput();this.sim.equipWeapon(id,actor,shown);if(changed)this.renderer.camera.weaponPitch=0;this.renderDirty=true;this.pendingWeapon=id;if(close)this.closeArsenal();this.toast(E.get(id).name+(id==='gauss'?' · mantén y suelta para disparar':id==='binoculars'?' · Z para observar, rueda para zoom':''));this.updateEquipmentHUD(performance.now());}
  paintArsenal(){
